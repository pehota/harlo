...
Today CLAUDE.md's checklist is read once at session start and drifts out of focus;
...
8. **Checklist outcomes not all green** → **BLOCK**. With no escalation, the recorded
   outcomes must pass: `tests.exit_code == 0`, every `review` finding addressed
   (`findings == addressed`), and every `task_checks[]` status `passed`. Otherwise block,
   naming the failed outcome.
9. **exit 0**.
...
structural. It cannot stop an agent from recording a false `0`, but that is now an
**explicit false record** in the transcript, not a silent omission — the same honesty lever
categories A/B rely on. The legitimate way past a red outcome is an **escalation** (step 7),
which carries its own evidence rules.
...
`done-write-state.sh` mirrors step 8: it **refuses to write** a non-passing done-state
unless an escalation field is present, giving the agent the feedback at `/done` time rather
than at stop time. The gate remains the structural backstop (it fires even if the file was
hand-written).
...
out of focus like CLAUDE.md; the agent records them into done-state `task_checks` when the
...
Invoke `/code-review` scoped to the step-1 diff. This spawns an **independent** reviewer
(not self-review, per MY_RULES §"don't grade your own homework").

### Step 5 — Address findings (bounded loop)

For each finding: fix, or explicitly justify as N/A. After fixes → **return to step 2**
(re-verify with fixes in place). A per-item counter enforces `max_fix_attempts`
(default 3); on exhaustion the item escalates via Category C.
...
The LLM supplies only the judgment fields (`dod`, `review`, `task_checks`, `escalation`,
`tests` summary) as a JSON payload. The script **injects the git facts live** —
`verified_sha = git rev-parse HEAD`, `tree_clean` from `git status --porcelain` — and
**refuses to write over a dirty tree** (so a "done" state can't be recorded atop
uncommitted changes). No hand-written SHA strings.
...
CLAUDE.md does** — so they are captured at **task start**, not recalled at the end.
...
1. `~/.claude/MY_RULES.md` → "Definition of Done" section
2. project `CLAUDE.md` / `.claude/CLAUDE.md`
3. task-stated verifications (→ `task_checks`)
...
**Evolution** — re-assembled on every `/done` run, so a change to `MY_RULES.md`,
`CLAUDE.md`, or the task is picked up automatically (same principle as config drift).
...
verified against it. It folds in the same external instructions (notably `MY_RULES.md`
§Definition of Done: real verification, changeset-scoped fresh-agent review, findings
addressed, build/tests green). The harness eats its own dog food.
