...
# "applicable". Nothing can exclude them: the harness reviews EVERY changed file,
# so the only excludable steps are 2-lint and 3, and only on their own
# per-config grounds (no command configured — nothing to run, not "out of scope").
...
emit_step() {
  ...
  jq -nc \
    ...
    --arg ref "dod-verify-protocol.md#$anchor" --arg reason "$reason" \
    'if ($reason | length) > 0
...
STEPS_JSON=$(
  {
    ...
    emit_step "7"      "Write the verification result (script)"  "$ALWAYS_STATUS" "$ALWAYS_REASON"
...
# `[id] <intent, padded> → dod-verify-protocol.md#<anchor>` per applicable step.
