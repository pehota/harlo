#!/bin/bash
#
# task-DoD plugin — the writer.
#
#   dod-write.sh <path-to-input-json>     (or: pipe the JSON on stdin)
#
# Writes / amends .claude/.harness/task-dod/<task_key>.json — the CONTRACT for
# this task. Not a hook; invoked by the model (or, in the tests, by a
# stand-in).
#
...
# task_key in the file is authoritative from the resolver (use HC_TASK_KEY
# verbatim); inject/overwrite it so the caller cannot mis-key.
