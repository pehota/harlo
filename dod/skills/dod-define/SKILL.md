...
touching any file. Same rule as D28's `/dod:verify` self-trigger. Skipping
it is a compliance failure, not a style choice; `track.sh`'s nudge and the
user running it themselves are the safety net for when you fail, not the
plan.
