#!/bin/bash
#
# Completion Harness — task-DoD walking skeleton: PostToolUse nudge.
#
...
# completion-harness/hooks.json invokes it.
#
...
#
# PostToolUse CANNOT block (issue target-flow: "PostToolUse nudge, ONCE"). The
# reminder goes out as `hookSpecificOutput.additionalContext` (agent-visible),
# the same channel baseline-snapshot.sh uses for its proactive steering — plus a
# `systemMessage` so it also shows in the user's terminal. Fail-safe is SILENT:
# any missing dependency → exit 0, no output.
#
...
# CLAUDE_PLUGIN_ROOT points at completion-harness/ when the plugin loads, so the
# shared lib is under scripts/ and the skeleton's own libs under dod/. Fall back
# to a path derived from this script's location for direct test invocation.
