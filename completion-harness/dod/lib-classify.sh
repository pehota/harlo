#!/bin/bash
#
# Completion Harness — task-DoD walking skeleton: path classifier (sourced lib).
#
# Part of the dod/ skeleton (issue #11 / ADR-0001). NOT wired into the live
# bundle. Sourced by dod-nudge.sh and dod-gate.sh; never executed directly.
#
# THE ONE RULE (issue "Terms" table): a changed path is PRODUCT surface unless
# it is matched by an `artifact_paths` glob. Fail CLOSED — an unlisted path
# (a brand-new top-level directory, say) is product, so a new area over-triggers
# the DoD rather than silently escaping the gate. The default list is
# deliberately directory-scoped and is NOT a blanket **/*.md: DOD.md and
# dod-protocol.md ARE the harness's behavioural contract and must never be
# exempted (ADR-0001 "Consequences").
#
# Glob semantics implemented below (documented, because the skeleton picks a
# concrete reading of the loose globs in the issue):
...
# task DoD, contradicting issue #11 "What is NOT the problem" (S0-idle exits
# quietly). This matches hc_state's own S0 logic. Committed range comes from
