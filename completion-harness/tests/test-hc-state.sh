...
# ---------------------------------------------------------------------------
# CASE S2-prose — committed changeset of ONLY prose files → S2, gated like any
#   other changeset. There is no file classification: what changed is reviewed,
#   whatever it is. Pins the removal of the old scope stand-down, which used to
#   drop this exact tree out of the state machine entirely.
# ---------------------------------------------------------------------------
echo
echo "--- CASE S2-prose: committed .md/.txt only STILL gates ---"
DIR=$(make_repo task)
commit_change "$DIR" "notes.md"   "# notes"   >/dev/null
commit_change "$DIR" "readme.txt" "hello"     >/dev/null
OUT=$(run_state "$DIR" "sess-prose-commit")
assert_eq       "S2-prose state" "$(split_state "$OUT")" "S2"
assert_nonempty "S2-prose next"  "$(split_next "$OUT")"

# ---------------------------------------------------------------------------
# CASE S1-prose — introduced prose dirt only (uncommitted) → S1. The
#   introduced-dirty guard sees a blocker and does not care what kind of file
#   it is.
# ---------------------------------------------------------------------------
echo
echo "--- CASE S1-prose: introduced .md dirt only STILL gates ---"
DIR=$(make_repo task)
echo "# wip notes" > "$DIR/wip.md"   # untracked → introduced blocker
OUT=$(run_state "$DIR" "sess-prose-dirt")
assert_eq       "S1-prose state" "$(split_state "$OUT")" "S1"
assert_nonempty "S1-prose next"  "$(split_next "$OUT")"
