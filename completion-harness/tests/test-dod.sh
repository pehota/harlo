#!/bin/bash
#
# Tests for the task-DoD walking skeleton (dod/ — issue #11 / ADR-0001).
#
# Covers all twelve situations from the issue's test matrix:
...
#
# Runs against the source-tree scripts (completion-harness/dod + scripts,
# resolved relative to this test) — no install needed. Each case builds an
# isolated throwaway git repo with CLAUDE_PROJECT_DIR / CLAUDE_PLUGIN_ROOT
# pointed at it. PASS/FAIL family (ok/bad/eq), matching sibling suites.
...
# tree baseline (.dirty) and .sha anchor exist for session-mode cases.
seed_session_baseline() {
  printf '{"session_id":"%s","source":"startup"}' "$2" \
    | CLAUDE_PROJECT_DIR="$1" bash "$SCRIPTS/baseline-snapshot.sh" >/dev/null 2>&1
}
...
# the session baseline stays quiet (issue #11 "What is NOT the problem").
# ---------------------------------------------------------------------------
R=$(make_repo)
...
# so lib-classify.sh and done-config.default.json can never drift.
# ---------------------------------------------------------------------------
DEF=$(jq -r '.artifact_paths | join(" ")' "$ROOT/contracts/done-config.default.json")
LIB=$( . "$DOD/lib-classify.sh"; printf '%s' "$DOD_DEFAULT_ARTIFACT_PATHS" )
eq "extra: lib-classify fallback == done-config.default.json artifact_paths" "$DEF" "$LIB"
