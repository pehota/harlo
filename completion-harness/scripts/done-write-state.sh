...
#
# The LLM supplies only JUDGMENT fields as a JSON payload on stdin (dod, review,
# task_checks, escalation, tests summary, app_started, ...). This script injects
# the git FACTS live — verified_sha = `git rev-parse HEAD`, tree_clean from
# `git status --porcelain` — so a SHA can never be hand-written, and it REFUSES
# to write over a dirty tree (a "done" state must not be recorded atop
# uncommitted changes).
#
...
# green: tests.exit_code == 0, review.findings == addressed, every task_check
...
if [ "$HAS_ESCALATION" != "yes" ]; then
  ...
  P_REVIEW=$(printf '%s' "$PAYLOAD" | jq -r '
    if (.review.findings != null and .review.addressed != null
        and (.review.findings == .review.addressed))
    then "green" else "notgreen" end
  ' 2>/dev/null)
  if [ "$P_REVIEW" != "green" ]; then
    echo "error: refusing to write — code-review findings are not all addressed; address them and re-run, or supply an escalation" >&2
    exit 1
  fi
