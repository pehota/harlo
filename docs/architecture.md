...
- **Fail-safe direction, restated.** Every uncertain path here **over**-includes:
  doubt keeps commits in the changeset and makes the gate demand a `/done`.
  ⚠ It does NOT follow that the window model is sound in either direction — that
  claim used to stand here and is FALSE. `commit-ledger.sh` enumerates four
  counter-examples: a **backgrounded** Bash call commits after the tool returns
  (closed by `.watermark`/`.bg-seen`), a **task-mode** commit the human later
  merges (closed by writing the ledger in task mode too), a **non-Bash** tool
  that moves HEAD (closed by the widened matcher), and — leaking the other way —
  a **fast-forward**, which moves HEAD *inside* a window and is still foreign
  (closed by the committer-date filter: a swept sha is claimed only when its
  `%ct` is at or after `baselines/<sid>.started`). An over-broad ledger is
  harmful too: it demands review of files the session never touched, so a
  pre-existing bug in someone else's merged code blocks unrelated work. `HC_BASE_ORIG` retains the *unadvanced*
...
|---|---|---|---|---|---|
| `baselines/<sid>.sha` | HEAD at SessionStart (session base) | **session id** | baseline-snapshot | rewritten each session (**not** on `source=compact` if present) | age 14d, **per-session GROUP** (all `<sid>.*` files reaped together, only when the NEWEST is stale) |
...
| `baselines/<sid>.dirty` | fork-point porcelain (session mode tree baseline); 0 bytes = genuinely clean at SessionStart | **session id** | baseline-snapshot | rewritten each session (**not** on `source=compact` if present); **atomic** temp+`mv`, and on a failed `git status` **no file is left** — a stale one is removed too | age 14d, **per-session GROUP** (all `<sid>.*` files reaped together, only when the NEWEST is stale) |
| `baselines/<sid>.own-commits` | commit ledger: SHAs whose creation was OBSERVED inside one of this session's Bash tool-call windows, one per line | **session id** | `commit-ledger.sh` (PostToolUse `Bash\|SlashCommand\|mcp__.*`, **both modes** — a task-mode commit the human later merges to trunk must not read as foreign) | **append-only, deduped**; created (empty) by the `pre` half on the first Bash call, then swept unconditionally from `CURSOR..HEAD` after *every* Bash call — no command-shape gate. Presence-vs-absence carries **no tier distinction any more**: absent and empty both mean "no agent commit observed", and `hc__commit_in_any_ledger` reads *all* sessions' ledgers, not just this one | age 14d, **per-session GROUP** (all `<sid>.*` files reaped together, only when the NEWEST is stale) |
| `baselines/<sid>.cursor.<tool_use_id>` (`.cursor` with no per-call id) | HEAD as it stood when THAT tool call started — the window's lower bound | **session id + tool_use_id** | `commit-ledger.sh pre` (PreToolUse `Bash\|SlashCommand\|mcp__.*`, **both modes**) | **one file per tool call**, written by its `pre` and read-then-REMOVED by its own `post`, so an open window is exactly a cursor file that still exists. This replaced a single shared `.cursor` (whole-file overwrite, last pin wins) under which concurrent windows narrowed each other: `pre(A) → commit → pre(B) → post → post` left the agent's own commit in NO ledger. Deliberately NOT an oldest-outstanding union across cursors — a `pre` whose `post` never fires (a denied tool call) would then widen every later sweep for the rest of the session and claim the human's between-window commits; per-call files make such an orphan inert. Only the no-id fallback path keeps the old last-pin-wins shape. Pinned externally, which is why the sweep needs no ancestor check: unlike the old ledger-tail cursor it has no feedback loop, so one orphaned sha cannot poison later calls. Missing at sweep time (an install with only PostToolUse wired, or a call denied before `pre` ran) → the sweep falls back to `baselines/<sid>.sha` and claims everything since SessionStart: a deliberate over-include, since over-claiming costs a spurious review demand while under-claiming silently skips one | age 14d, **per-session GROUP** (all `<sid>.*` files reaped together, only when the NEWEST is stale) |
| `baselines/<sid>.watermark` | the last HEAD a sweep actually reached | **session id** | `commit-ledger.sh` (post) | overwritten after every completed sweep, **including a zero-length one** (HEAD unmoved) — that no-move write is what a backgrounded call produces, and without it the `pre` fallback below never engages | age 14d, **per-session GROUP** (all `<sid>.*` files reaped together, only when the NEWEST is stale) |
| `baselines/<sid>.bg-seen` | marker: this session backgrounded a Bash call, so a commit of its own can land OUTSIDE every call window (PostToolUse fires when the tool returns, not when the shell exits) | **session id** | `commit-ledger.sh pre` | created once, **sticky for the session's life**. While present, `pre` pins the cursor to `.watermark` instead of current HEAD — an over-block confined to sessions that really backgrounded something. Absent (the normal case) keeps "HEAD moved outside every window ⇒ foreign" sound, which is what lets a pure Q&A session Stop cleanly | age 14d, **per-session GROUP** (all `<sid>.*` files reaped together, only when the NEWEST is stale) |
| `baselines/<sid>.started` | the session's start time, **epoch seconds in the CONTENT** (never mtime — baseline mtime is a known mutable false-PASS risk) | **session id** | `baseline-snapshot.sh` (SessionStart) | written **only when absent**, never re-stamped: a resume that moved the epoch forward could filter out a commit the session really authored but has not swept yet. It is the committer-date floor for the sweep — `commit-ledger.sh` appends a swept sha only when its `%ct` is >= this epoch, which is what stops a `git pull --ff-only` inside a window from being recorded as authorship. Absent/empty/unparseable ⇒ **no filtering** (over-block, the safe direction at the append layer). Residual: a colleague pushing DURING the session is still swept in | age 14d, **per-session GROUP** (all `<sid>.*` files reaped together, only when the NEWEST is stale) |
...
SESSION-mode state — but `baselines/` is aged **per session id, all-or-nothing**, not per file:
a session's files are one unit of state with ASYMMETRIC mtimes (`.watermark`/`.own-commits` are
rewritten constantly, while `.bg-seen`/`.sweep-failed` are written ONCE and freeze), so a per-file
age test deleted the frozen SAFETY MARKERS and spared the state they qualify — re-enabling both
silent skips they exist to prevent. The group is reaped only when its NEWEST member is stale, and
then entirely; any future per-session file inherits the rule. (2) **Terminal reap** —
...
| **Mid-work rebase / amend / cherry-pick (session mode)** | Rewriting history changes a commit's SHA while preserving its content, so a rewritten agent commit `A → A'` is in no ledger and would read as foreign — dropping **real agent work out of the changeset**, the one direction the harness must never fail in. `hc__ledger_history_rewritten` runs **before** any advance: every sha in *this session's* ledger must still `cat-file -e` **and** be an ancestor of HEAD. Any failure ⇒ **no advance at all** ⇒ full changeset ⇒ gate engages. Both halves are **batched into exactly two git calls** whatever the ledger size (`cat-file --batch-check` fed every sha, then `rev-list --stdin ^HEAD`) — existence FIRST, because `rev-list` errors out on a missing object instead of reporting it. It used to be two forks *per ledger line*, which spent the Stop hook's whole 10s budget at a couple of hundred entries, and one in-window `git pull` on a repo well behind trunk produces a ledger that long in one go. A **second, parallel tripwire** with the same over-block: `baselines/<sid>.sweep-failed` (`commit-ledger.sh` could not complete a sweep — a destroyed cursor object *and* a failed `.sha`-baseline retry). A sweep we could not complete is **unknown attribution, not absence of work**. ⚠ The rewrite tripwire suppresses the base ADVANCE only. `.sweep-failed` does BOTH: it also makes `hc_review_coverage_gap` treat the ledger as NOT known-complete and fall back to the whole range diff — otherwise the reviewer attested only the ledgered files, the coverage gap came back empty, and the very window whose attribution was lost shipped unreviewed. It does not recover the attribution, it refuses to guess — a silent skip converted into an over-block. Scope is this session's ledger only (peer ledgers routinely hold shas unreachable from our HEAD, e.g. deleted branches; tripping on those would block every session forever). **Deferred:** content identity via `git patch-id` stored beside each sha would actually recover the attribution — a ledger format change with its own migration, out of scope here | `hc__ledger_history_rewritten`; `hc__resolve_session_base` (early return) |
| **Foreign commit inside the agent's own long Bash call** | **Accepted residual.** A hand-commit landing during, say, a 3-minute test run is inside that call's pre→post window and gets swept into the ledger, so it is misattributed as agent work and costs a spurious `/done` demand. Ceiling on the damage: this race is an **over**-block, never a skipped review. That ceiling belongs to THIS race only — "HEAD moving outside every window ⇒ foreign" is FALSE (four counter-examples in `commit-ledger.sh`: backgrounded call, task-mode commit later merged, non-Bash tool, and a fast-forward, which leaks the *other* way by moving HEAD INSIDE a window over foreign commits). Concurrent same-tree sessions are already unsupported-racy (see "Parallel same-branch"). Upgrade path if it ever bites: correlate `git reflog HEAD` entries with the window instead of a bare two-point HEAD diff, or have the agent's commit path stamp the sha directly | `commit-ledger.sh` (`ponytail:` accepted-residual note); `hc__commit_in_any_ledger` |
...
| File | Responsibility |
|---|---|
| `scripts/harness-common.sh` | Sourced lib: `hc_resolve` (identity/base/tree-base + authorship base-advance in session mode, `HC_BASE_ORIG`), `hc_tree_status` (tree classifier), `hc_tree_remediation`, `hc_review_blocking` (severity-gated review count), `hc_review_coverage_gap` (BLOB-keyed coverage across the log chain; 7 args — `extra_admit`/`chain_admit` anchor admission, trailing `[orig_base]` engages the ledger-set scope: changed-set = `hc_session_changeset_files` union when a session id is live, `orig_base` is non-empty AND the ledger is **known complete** — i.e. NOT task mode (whose pinned fork point deliberately never drops a foreign commit) and neither tripwire fires for any id that could name this session (`hc__guard_session_ids` = `HC_SESSION_ID` **and** the `current-session` marker, because on the gate's Step-2a-0 id-disagreement path the hook wrote `.sweep-failed` under the OTHER id): no `baselines/<id>.sweep-failed`, no `hc__ledger_history_rewritten`, and an id that resolves at all; otherwise the `<base>..HEAD` range diff, since the ledger set NARROWS the demand and an incomplete ledger would exempt files from review; the chain-walk filter shares `orig_base` as its lower bound), `hc_session_changeset_commits` / `hc_session_changeset_files` (emit / union the agent-authored SHAs in `orig_base..HEAD` — the DoD scope set, dropping interior foreign commits), `hc__commit_in_any_ledger` (THE authorship predicate: ledger membership across all sessions — the union of every `*.own-commits` is built ONCE per range walk by `hc__ledger_set_load` into a newline-framed blob and matched in-process — zero forks per commit, one fork per ledger FILE for the load itself (measured at HEAD: 40 files 0.32s, 200 2.02s, 500 9.24s, 1000 30.4s — a surviving file-count ceiling `caseP` does not guard, since it pins lines at 5851 but files at 40); it used to fork one `grep` per ledger file per commit, ~8s of the Stop hook's 10s budget at 40 peers over a 30-commit range, and the first fix for that was quadratic in ledger LINES (5851 lines = 12.39s) while still claiming "zero forks") + `hc__commit_session_authored` (thin alias) + `hc__ledger_history_rewritten` (reachability tripwire — refuse to advance when our own ledgered shas stop being reachable), `hc_changeset_summary` (block-message summary), `hc_live_task_keys` (terminal-reap keep-set), `hc_live_review_shas` (review-log hygiene keep-set), `hc_validate` (jq-only JSON-Schema-subset validator, fail-closed on unsupported keywords; sets `HC_CONTRACTS_DIR`), `hc_done_state_blocked` + `hc_state` (composed operator state S0/S1/S2/S4/S5) |
